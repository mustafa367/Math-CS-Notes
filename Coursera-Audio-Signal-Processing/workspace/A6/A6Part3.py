...
"""
def estimateInharmonicity2(inputFile = '../../sounds/piano.wav', t1=0.1, t2=0.5, window='hamming', 
...
def estimateInharmonicity(inputFile = '../../sounds/piano.wav', t1=0.1, t2=0.5, window='hamming', 
    ...
    bin1 = int(np.ceil((t1 / total_time) * len(hfreq)))
    bin2 = int(np.ceil((t2 / total_time) * len(hfreq)))

    bhfreq = hfreq[bin1:bin2]
    bf0 = f0[bin1:bin2]
    ...
    meanInharm = np.sum(inhm) / len(inhm)

    return meanInharm

if __name__=="__main__":
    print(estimateInharmonicity(inputFile = '../../sounds/piano.wav', t1=0.2,
                                t2=0.4, window='hamming', M=2047, N=2048, H=128, f0et=5.0, t=-90, minf0=130,
                                maxf0=180, nH = 25))
    print(estimateInharmonicity(inputFile = '../../sounds/piano.wav', t1=2.3,
                                t2=2.55, window='hamming', M=2047, N=2048, H=128, f0et=5.0, t=-90, minf0=230,
                                maxf0=290, nH = 15))
    print(estimateInharmonicity(inputFile = '../../sounds/piano.wav', t1=2.55,
